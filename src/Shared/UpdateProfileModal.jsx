...
function UpdateProfileModal({ setUpdateOpen, open }) {
  ...
  const [phone, setPhone] = useState(
    currentUser?.phone ? currentUser?.phone : "8801....."
  );
  const [email, setEmail] = useState(currentUser?.email);
  const [address, setAddress] = useState("Nalitabari serpur");
  const [bio, setBio] = useState(
    currentUser?.bio ? currentUser?.bio : "Enter Your bio data"
  );
    ...
    fetch(url, {
      ...
      .then(result => {
        if (result.success) {
          ...
          // send data backend
          fetch(
            `https://zoomla-backend.herokuapp.com/api/users/${currentUser?._id}`,
            {
              method: "PUT",
              headers: {
  ...
  return (
    <Fragment>
      <Dialog
        className={`${
          dark ? "!bg-[#4b69c294]" : "!bg-[#0509157a]"
        } flex items-center justify-end sm:justify-center !w-full`}
        size="xxl"
        open={open}
      ...
      >
        <div
          className={`${
            dark ? "!bg-[#1c1f2e] text-white" : "!bg-[#fff] text-[#000]"
          } w-full sm:w-[60%] lg:w-[40%] max-h-[620px] overflow-y-auto rounded-t-2xl sm:!rounded-2xl p-3 sm:p-6`}
        >
          ...
          <form onSubmit={handleUpdate}>
            <DialogBody
              className={`${
                dark ? "border-[#3f445d]" : "border-[#e8eaf5]"
              } !p-0 !w-full !block border-y`}
            >
                  ...
                  <div className="flex text-center justify-between items-center">
                    ...
                    <div className=" w-full ml-3">
                      <input
                        ...
                        className={`${
                          dark
                            ? "border-[#2d303d] bg-[#212534] text-[#b1afaf]"
                            : "text-[#0e78f9] border-[#c2daf8]"
                        }  border text-sm !w-full px-4 !py-[8px] rounded-[14px] mb-3 outline-0`}
                      />
                      <input
                        type="text"
                        required
                        ...
                        className={`${
                          dark
                            ? "border-[#2d303d] bg-[#212534] text-[#b1afaf]"
                            : "text-[#0e78f9] border-[#c2daf8]"
                        }  border text-sm !w-full px-4 !py-[8px] rounded-[14px] outline-0`}
                      />
                    </div>
                  </div>
                    ...
                    <textarea
                      ...
                      className={`${
                        dark
                          ? "border-[#2d303d] bg-[#212534] text-[#b1afaf]"
                          : "text-[#0e78f9] border-[#c2daf8]"
                      } border text-sm !w-full px-4 !py-[8px] rounded-[10px] outline-0`}
            ...
            <DialogFooter className="text-left w-full">
              <div className="flex items-center">
                <Button
                  ...
                  className={` ${
                    dark
                      ? "bg-[#282c3a] text-gray-500"
                      : "!bg-[#eff6ff] text-[#0e78f9]"
                  } !px-4 capitalize flex items-center`}
                  variant="text"
                >
                  Cancel
                </Button>
