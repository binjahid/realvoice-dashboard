      ...
      {FeedPosts?.map((p, index) => (
        <div key={index}>
          <motion.div
            ...
            className={` ${
              dark
                ? "bg-[#212534] border-[#262938] text-white"
                : "bg-[#eff6ff] border-[#e8eaf5] text-[#000]"
            } rounded-[24px] w-full h-auto text-white border p-[10px]`}
          >
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  ...
                  <small className="text-xm text-gray-600 block mt-[-3px]">
                    {PostUser(p.email)?.address}
                  </small>
                </div>
              </div>
              <Menu placement="bottom-end">
                ...
                <MenuList
                  onClick={() => handleOpen(p, "update")}
                  className={`${
                    dark
                      ? "bg-[#272b39] text-gray-400"
                      : "bg-[#fff] text-[#000]"
                  } shadow-xl border-transparent`}
                >
                  <MenuItem
                    className={` ${
                      dark ? "bg-[#272b39] hover:bg-[#272b39]" : "bg-[#fff]"
                    } hover:shadow-md`}
                  >
                    Edit
                  </MenuItem>
            ...
            >
              <ul className="flex items-center gap-x-3">
                <li
                  onClick={() => handleLike(currentUser?.email, p)}
                ...
                >
                  <span>
                    ...
                    currentUser?.email ===
                      p?.likes?.find(l =>
                        l?.email?.includes(currentUser?.email)
                      )?.email ? (
                      <FcLike />
                    ) : (
                      <FcLikePlaceholder />
                ...
                <li className=" text-[20px] cursor-pointer">
                  ...
                  >
                    ...
                    <Popover.Dropdown
                      className={`${
                        dark ? "bg-[#242836] border-[#383e55]" : "bg-[#fff]"
                      } p-1 rounded-xl`}
                    >
                      <LoadingOverlay visible={visible} overlayBlur={2} />
                      <form
                        onSubmit={e =>
                          handleComment(e, currentUser?.email, p?._id)
                        }
                      >
                        <textarea
                          required
                          ...
                          className={`${
                            dark ? "bg-[#242836] border-[#383e55]" : "bg-[#fff]"
                          } !text-gray-500 border text-sm !w-full px-3 !py-[5px] rounded-[10px] outline-0`}
                        ...
                        <div className="mb-2 flex justify-end">
                          ...
                          <input
                            type="submit"
                            value="Sent"
                            className="!text-white border text-sm px-4 !py-[5px] cursor-pointer rounded-[14px] outline-0 border-[#8699ef] bg-[#3a7ef4]"
                          />
                        </div>
                      </form>
                    </Popover.Dropdown>
            ...
            <div className="my-2">
                ...
                {p?.likes?.length > 0 && (
                  ...
                  >
                    ...
                    <span className="capitalize">
                      {" "}
                      {p?.likes?.[p?.likes?.length - 1]?.email.slice(0, 5)}{" "}
                    </span>
                    {p?.likes?.length > 1 && (
                      <span>and {p?.likes?.length - 1} others</span>
                    )}
                  </p>
                )}
                {p?.likes?.length === 0 && (
              ...
              <div className="flex items-center">
                ...
                {p?.title?.length > 40 && (
                  <p className="text-sm cursor-pointer text-blue-gray-500">
                    ...more
                  </p>
                )}
              </div>
              {p?.comments && (
                ...
                >
                  {PostUser(p?.comments?.[p?.comments.length - 1]?.email)
                    ?.img && (
                    <div>
                      <img
                        src={
                  ...
                  <p
                    ...
                    } font-bold mx-1 capitalize text-sm`}
                  >
                    {p?.comments?.[p?.comments.length - 1]?.email?.slice(0, 5)}{" "}
                  </p>
                  <p className="text-sm text-gray-500">
                    {" "}
                    {p?.comments?.[p?.comments.length - 1]?.comment.slice(
                      0,
                      40
                    )}
                  </p>
